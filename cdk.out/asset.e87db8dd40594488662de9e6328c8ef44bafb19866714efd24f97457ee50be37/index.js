...
exports.handler = async (event) => {
  ...
  if (method === 'GET' && idParam && event.rawPath.endsWith('/files')) {
    ...
    const params = {
      TableName: TABLE_NAME,
      KeyConditionExpression: 'pk = :pk AND begins_with(sk, :pref)'.replace(':pref', 'FILE#'),
      ExpressionAttributeValues: {
        ':pk': `PROJECT#${idParam}`,
      },
      Limit: Math.min(Math.max(1, limit), 200),
    };
    // Use FilterExpression for type if provided
    if (type) {
    ...
    // Adjust KeyConditionExpression since we need begins_with on sk
    params.KeyConditionExpression = 'pk = :pk AND begins_with(sk, :skPrefix)';
    params.ExpressionAttributeValues[':skPrefix'] = 'FILE#';
  ...
  if (method === 'POST' && idParam && event.rawPath.endsWith('/files')) {
